...
class ModuleWorld:
    def __init__(self, name, args, timeout, module_manager, width, height):
        ...
        self.init_s = 300  # ego initial s location
        self.init_d = -1 * self.LANE_WIDTH  # ego initial lane number - int range: [-1, 2]  => change in reset function
    ...
    def _spawn_hero(self):
        ...
        self.imu_sensor = IMUSensor(self.hero_actor)
    ...
    def reset(self):
        ...
        # self.init_s = np.random.uniform(50, self.max_s - self.track_length - 50)  # ego initial s location # zjj
        ...
        self.init_d = -1 * self.LANE_WIDTH#np.random.randint(-1, 3) * self.LANE_WIDTH  # -1 and 3 because global route is defined on the second lane from left
        # print("init_d:{}".format(self.init_d))

        # print("ego_car init frenet s:{}, d:{}".format(self.init_s, self.init_d))
        x, y, z, yaw = frenet_to_inertial(self.init_s, self.init_d, self.global_csp)
        # print("ego_car init x:{}, y:{}".format(x, y))
        z += 0.1
    ...
    def tick(self):
        ...
        self.imudata = self.imu_sensor.get_imu_data()
...
class TrafficManager:
    ...
    def reset(self, ego_s, ego_d):
        ...
        grid_choices = np.array([16, 17, 20, 21, 24, 25, 28, 29, 32, 33, 36, 37, 40, 41, 44, 45, 48, 49, 52, 53, 56, 57, 60])
        ...
        for idx in rnd_indices:
            ...
            s = ego_s + col * 15 - 30  # -20 bc ego is on second column
            targetSpeed = random.uniform(self.min_speed, self.max_speed)  # m/s
            if idx == 40:
                targetSpeed = 0.01
            self.spawn_one_actor(s, lane, targetSpeed)

        # rnd_indices = np.array([23, 46, 53, 59, 44, 41, 42, 40, 39, 52, 20, 57, 49, 26, 19])
        # rnd_velocity = np.array([9.249745946837193,
        #                         11.009120098869428,
        #                         8.846086697891785,
        #                         10.86476699550728,
        #                         10.186748906461427,
        ...
        #                         9.273592010073507,
        #                         10.74074187812406])
        # for index, idx in enumerate(rnd_indices):
        #     col = idx // 4  # col number [0, 19]
        #     lane = idx - col * 4 - 1  # lane number [-1, 2]
        #     s = ego_s + col * 15 - 30  # -20 bc ego is on second column
        #     targetSpeed = rnd_velocity[index]  # m/s
        #     self.spawn_one_actor(s, lane, targetSpeed)

        # self.spawn_one_actor(ego_s, ego_lane - 1, 1)
        # self.spawn_one_actor(ego_s, ego_lane + 1, 1)
        # self.spawn_one_actor(ego_s, ego_lane + 2, 1)
        # self.spawn_one_actor(ego_s - 16, ego_lane, 1)
        # self.spawn_one_actor(ego_s - 16, ego_lane - 1, 1)
        # self.spawn_one_actor(ego_s - 32, ego_lane + 1, 1)
        # self.spawn_one_actor(ego_s - 16, ego_lane + 2, 1)
...
class IMUSensor(object):
    def __init__(self, parent_actor):
        ...
        weak_self = weakref.ref(self)
        self.sensor.listen(lambda imu_data: IMUSensor.on_received_imu_data(weak_self, imu_data))

    @staticmethod
    def on_received_imu_data(weak_self, imu_data):
        self = weak_self()
        if not self:
            return
