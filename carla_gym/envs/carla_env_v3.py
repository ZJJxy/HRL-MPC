...
class CarlaGymEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.deq_leading_v = deque(maxlen=self.DEQSIZE)
        self.deq_follow_v = deque(maxlen=self.DEQSIZE)
        self.deq_left_v = deque(maxlen=self.DEQSIZE)
        self.deq_leftup_v = deque(maxlen=self.DEQSIZE)
        self.deq_leftdown_v = deque(maxlen=self.DEQSIZE)
        self.deq_lleft_v = deque(maxlen=self.DEQSIZE)
        self.deq_lleftup_v = deque(maxlen=self.DEQSIZE)
        self.deq_llftdown_v = deque(maxlen=self.DEQSIZE)
        self.deq_right_v = deque(maxlen=self.DEQSIZE)
        self.deq_rightup_v = deque(maxlen=self.DEQSIZE)
        self.deq_rightdown_v = deque(maxlen=self.DEQSIZE)
        self.deq_rright_v = deque(maxlen=self.DEQSIZE)
        self.deq_rrightup_v = deque(maxlen=self.DEQSIZE)
        self.deq_rrightdown_v = deque(maxlen=self.DEQSIZE)
        self.deq_vlist = [self.deq_ego_speed, self.deq_leading_v, self.deq_follow_v, self.deq_left_v,
                         self.deq_leftup_v, self.deq_leftdown_v, self.deq_lleft_v,self.deq_lleftup_v,
                         self.deq_llftdown_v, self.deq_right_v,self.deq_rightup_v, self.deq_rightdown_v,
                         self.deq_rright_v, self.deq_rrightup_v, self.deq_rrightdown_v]
    ...
    def enumerate_actors(self, ego_s, ego_d, ego_deq_s, ego_deq_d):
        ...
        for i, actor_id in enumerate(self.actor_enumeration):
            if actor_id >= 0:
                ...
                vehicle = actor_id_vehicle[actor_id]
                v = get_speed(vehicle)
                actor_v.append(v / self.maxSpeed)
        ...
        if norm_s[0] not in (-1, -2):
            enumerated_dict['LEADING'] = {'S': norm_s[0], 'v': actor_v[0]}
        else:
            enumerated_dict['LEADING'] = {'S': [emp_ln_max], 'v': emp_v_max}

        if norm_s[1] not in (-1, -2):
            enumerated_dict['FOLLOWING'] = {'S': norm_s[1], 'v': actor_v[1]}
        else:
            enumerated_dict['FOLLOWING'] = {'S': [emp_ln_min], 'v': emp_v_min}

        if norm_s[2] not in (-1, -2):
            enumerated_dict['LEFT'] = {'S': norm_s[2], 'v': actor_v[2]}
        else:
            enumerated_dict['LEFT'] = {'S': [emp_ln_min] if norm_s[2] == -1 else [no_ln], 'v': emp_v_min if norm_s[2] == -1 else no_ln}

        if norm_s[3] not in (-1, -2):
            enumerated_dict['LEFT_UP'] = {'S': norm_s[3], 'v': actor_v[3]}
        else:
            enumerated_dict['LEFT_UP'] = {'S': [emp_ln_max] if norm_s[3] == -1 else [no_ln_up], 'v': emp_v_max if norm_s[3] == -1 else no_ln_up}

        if norm_s[4] not in (-1, -2):
            enumerated_dict['LEFT_DOWN'] = {'S': norm_s[4], 'v': actor_v[4]}
        else:
            enumerated_dict['LEFT_DOWN'] = {'S': [emp_ln_min] if norm_s[4] == -1 else [no_ln_down], 'v': emp_v_min if norm_s[4] == -1 else no_ln_down}

        if norm_s[5] not in (-1, -2):
            enumerated_dict['LLEFT'] = {'S': norm_s[5], 'v': actor_v[5]}
        else:
            enumerated_dict['LLEFT'] = {'S': [emp_ln_min] if norm_s[5] == -1 else [no_ln], 'v': emp_v_min if norm_s[5] == -1 else no_ln}

        if norm_s[6] not in (-1, -2):
            enumerated_dict['LLEFT_UP'] = {'S': norm_s[6], 'v': actor_v[6]}
        else:
            enumerated_dict['LLEFT_UP'] = {'S': [emp_ln_max] if norm_s[6] == -1 else [no_ln_up], 'v': emp_v_max if norm_s[6] == -1 else no_ln_up}

        if norm_s[7] not in (-1, -2):
            enumerated_dict['LLEFT_DOWN'] = {'S': norm_s[7], 'v': actor_v[7]}
        else:
            enumerated_dict['LLEFT_DOWN'] = {'S': [emp_ln_min] if norm_s[7] == -1 else [no_ln_down], 'v': emp_v_min if norm_s[7] == -1 else no_ln_down}

        if norm_s[8] not in (-1, -2):
            enumerated_dict['RIGHT'] = {'S': norm_s[8], 'v': actor_v[8]}
        else:
            enumerated_dict['RIGHT'] = {'S': [emp_ln_min] if norm_s[8] == -1 else [no_ln], 'v': emp_v_min if norm_s[8] == -1 else no_ln}

        if norm_s[9] not in (-1, -2):
            enumerated_dict['RIGHT_UP'] = {'S': norm_s[9], 'v':actor_v[9]}
        else:
            enumerated_dict['RIGHT_UP'] = {'S': [emp_ln_max] if norm_s[9] == -1 else [no_ln_up], 'v': emp_v_max if norm_s[9] == -1 else no_ln_up}

        if norm_s[10] not in (-1, -2):
            enumerated_dict['RIGHT_DOWN'] = {'S': norm_s[10], 'v': actor_v[10]}
        else:
            enumerated_dict['RIGHT_DOWN'] = {'S': [emp_ln_min] if norm_s[10] == -1 else [no_ln_down], 'v': emp_v_min if norm_s[10] == -1 else no_ln_down}

        if norm_s[11] not in (-1, -2):
            enumerated_dict['RRIGHT'] = {'S': norm_s[11], 'v': actor_v[11]}
        else:
            enumerated_dict['RRIGHT'] = {'S': [emp_ln_min] if norm_s[11] == -1 else [no_ln], 'v': emp_v_min if norm_s[11] == -1 else no_ln}

        if norm_s[12] not in (-1, -2):
            enumerated_dict['RRIGHT_UP'] = {'S': norm_s[12], 'v': actor_v[12]}
        else:
            enumerated_dict['RRIGHT_UP'] = {'S': [emp_ln_max] if norm_s[12] == -1 else [no_ln_up], 'v': emp_v_max if norm_s[12] == -1 else no_ln_up}

        if norm_s[13] not in (-1, -2):
            enumerated_dict['RRIGHT_DOWN'] = {'S': norm_s[13], 'v': actor_v[13]}
        else:
            enumerated_dict['RRIGHT_DOWN'] = {'S': [emp_ln_min] if norm_s[13] == -1 else [no_ln_down], 'v': emp_v_min if norm_s[13] == -1 else no_ln_down}
    ...
    def Dynamic_state_update(self, dict):
        ...
        self.deq_leading_v.append(dict['LEADING']['v'])
        self.deq_follow_v.append(dict['FOLLOWING']['v'])
        self.deq_left_v.append(dict['LEFT']['v'])
        self.deq_leftup_v.append(dict['LEFT_UP']['v'])
        self.deq_leftdown_v.append(dict['LEFT_DOWN']['v'])
        self.deq_lleft_v.append(dict['LLEFT']['v'])
        self.deq_lleftup_v.append(dict['LEFT_UP']['v'])
        self.deq_llftdown_v.append(dict['LLEFT_DOWN']['v'])
        self.deq_right_v.append(dict['RIGHT']['v'])
        self.deq_rightup_v.append(dict['RIGHT_UP']['v'])
        self.deq_rightdown_v.append(dict['RIGHT_DOWN']['v'])
        self.deq_rright_v.append(dict['RRIGHT']['v'])
        self.deq_rrightup_v.append(dict['RRIGHT_UP']['v'])
        self.deq_rrightdown_v.append(dict['RRIGHT_DOWN']['v'])
        ...
        self.actor_dyenumerated_dict['EGO']['SPEED'] = list(self.deq_ego_speed)
        self.actor_dyenumerated_dict['LEADING']['V'] = list(self.deq_leading_v)
        self.actor_dyenumerated_dict['FOLLOWING']['V'] = list(self.deq_follow_v)
        self.actor_dyenumerated_dict['LEFT']['V'] = list(self.deq_left_v)
        self.actor_dyenumerated_dict['LEFT_UP']['V'] = list(self.deq_leftup_v)
        self.actor_dyenumerated_dict['LEFT_DOWN']['V'] = list(self.deq_leftdown_v)
        self.actor_dyenumerated_dict['LLEFT']['V'] = list(self.deq_lleft_v)
        self.actor_dyenumerated_dict['LLEFT_UP']['V'] = list(self.deq_lleftup_v)
        self.actor_dyenumerated_dict['LLEFT_DOWN']['V'] = list(self.deq_llftdown_v)
        self.actor_dyenumerated_dict['RIGHT']['V'] = list(self.deq_right_v)
        self.actor_dyenumerated_dict['RIGHT_UP']['V'] = list(self.deq_rightup_v)
        self.actor_dyenumerated_dict['RIGHT_DOWN']['V'] = list(self.deq_rightdown_v)
        self.actor_dyenumerated_dict['RRIGHT']['V'] = list(self.deq_rright_v)
        self.actor_dyenumerated_dict['RRIGHT_UP']['V'] = list(self.deq_rrightup_v)
        self.actor_dyenumerated_dict['RRIGHT_DOWN']['V'] = list(self.deq_rrightdown_v)
        ...
        lstm_obs_v = np.concatenate((np.array(self.actor_dyenumerated_dict['EGO']['SPEED'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LEADING']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['FOLLOWING']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LEFT']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LEFT_UP']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LEFT_DOWN']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LLEFT']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LLEFT_UP']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['LLEFT_DOWN']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['RIGHT']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['RIGHT_UP']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['RIGHT_DOWN']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['RRIGHT']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['RRIGHT_UP']['V'])[_range],
                                   np.array(self.actor_dyenumerated_dict['RRIGHT_DOWN']['V'])[_range]),
                                  axis=0)
        # state = np.array([lstm_obs.reshape(self.observation_space.shape[1], -1).transpose(), lstm_obs_v.reshape(self.observation_space.shape[1], -1).transpose()])
        state = lstm_obs.reshape(self.observation_space.shape[1], -1).transpose()
        return state  # state shape:(5,15)
    ...
    def step(self, action=None):
        ...
        # action[0]：-1，0，1 左，直行，右
        # action[1]：0，1，2 三个终点
        fpath, self.lanechange, off_the_road, v_exp = self.motionPlanner.run_step_single_path(ego_state, self.f_idx, df_n=action, Tf=5, Vf_n=action)
        ...
        while self.f_idx < wps_to_go and (elapsed_time(path_start_time) < self.motionPlanner.D_T * 1.5 or
            ...
            imu_accelerate = self.world_module.imudata.accelerometer
            imu_gyro = self.world_module.imudata.gyroscope
            ax_pre = imu_accelerate.x
            ay_pre = imu_accelerate.y
            az_pre = imu_accelerate.z
            wz_pre = imu_gyro.z
            ...
            # cmdSpeed = 11.11
            # cmdSpeed = self.targetSpeed*0.75
            # if self.lanechange:
            ...
            Jerk_cur = math.sqrt(ax**2 + ay**2 + az**2)
            Jerk_pre = math.sqrt(ax_pre**2 + ay_pre**2 + az_pre**2)
            J_comfort = (Jerk_cur - Jerk_pre) / self.dt
            delta_wz = (wz - wz_pre) / self.dt
            if self.lanechange and self.f_idx < len(fpath.t) * 0.8:
                with open("./data_Excel/testdata_step_Jerk_wz.csv", "a+", newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([self.n_step, float(J_comfort), float(delta_wz), int(collision)])
            ...
            last_speed = get_speed(self.ego)
            # with open("./data_Excel/traindata_step_tarV_actV_tx_ty_ax_ay.csv", "a+", newline='') as f:
            #     writer = csv.writer(f)
            #     writer.writerow([self.n_step, float(cmdSpeed), float(last_speed), float(fpath.x[self.f_idx]),
            #                      float(fpath.y[self.f_idx]), float(self.ego.get_location().x), float(self.ego.get_location().y)])
        ...
        with open("./data_Excel/traindata_step_velocity.csv", "a+", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([self.n_step, float(last_speed), int(collision)])
        ...
        if done == False:
            ...
            with open("./data_Excel/eps_epsR.csv", "a+", newline='') as f:
                writer = csv.writer(f)
                writer.writerow([self.episode, float(self.eps_rew)])
